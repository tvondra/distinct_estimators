...
CREATE FUNCTION hyperloglog_size(nbitmaps int, keysize int) RETURNS int
...
CREATE FUNCTION hyperloglog_init(nbitmaps int, keysize int) RETURNS hyperloglog_estimator
...
CREATE FUNCTION hyperloglog_add_item_agg(counter hyperloglog_estimator, item text, nbitmaps integer, keysize integer) RETURNS hyperloglog_estimator
     AS 'MODULE_PATHNAME', 'hyperloglog_add_item_agg_text'
     LANGUAGE C;

CREATE FUNCTION hyperloglog_add_item_agg(counter hyperloglog_estimator, item int, nbitmaps integer, keysize integer) RETURNS hyperloglog_estimator
...
CREATE AGGREGATE hyperloglog_distinct(text, int, int)
(
...
CREATE AGGREGATE hyperloglog_distinct(int, int, int)
(
    sfunc = hyperloglog_add_item_agg,
    stype = hyperloglog_estimator,
    finalfunc = hyperloglog_get_estimate
