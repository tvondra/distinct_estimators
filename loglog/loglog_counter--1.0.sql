...
CREATE FUNCTION loglog_add_item_agg(counter loglog_estimator, item text, nbitmaps integer, keysize integer) RETURNS loglog_estimator
     AS 'MODULE_PATHNAME', 'loglog_add_item_agg_text'
     LANGUAGE C;

CREATE FUNCTION loglog_add_item_agg(counter loglog_estimator, item int, nbitmaps integer, keysize integer) RETURNS loglog_estimator
...
CREATE AGGREGATE loglog_distinct(text, int, int)
(
...
CREATE AGGREGATE loglog_distinct(int, int, int)
(
    sfunc = loglog_add_item_agg,
    stype = loglog_estimator,
    finalfunc = loglog_get_estimate
